...
class ApplicationUI {
    ...
    userMenu(username, role) {
		...
		do {
			option = prompt("Seleccione una opcin:\n1. Cambiar contraseña\n2. Ver artículos\n3. Agregar artículo\n4. Editar artículo\n5. Eliminar artículo\n6. Comprar artículos\nX. Salir");
			...
			switch (option) {
				case '1':
					...
					if (userData.password === currentPassword) {
						...
						if (this.model.isValidPassword(newPassword)) {
							...
							alert("La contraseña no cumple con los requisitos de seguridad:\n- 8 a 16 caracteres\n- Al menos 1 mayuscula\n- Al menos 2 simbolos especiales");
						}
					} else {
				...
				case '2':
					if (this.model.hasPermission(role, 'canView')) {
						...
						alert("No tiene permiso para ver los articulos.");
					}
					break;
				case '3':
					if (this.model.hasPermission(role, 'canAdd')) {
						...
						alert("No tiene permiso para agregar articulos.");
					}
					break;
				case '4':
					if (this.model.hasPermission(role, 'canEdit')) {
						...
						alert("No tiene permiso para editar articulos.");
					}
					break;
				case '5':
				...
				case '6':
					if (this.model.hasPermission(role, 'canBuy')) {
						...
						alert("No tiene permiso para comprar articulos.");
					}
					break;
				case 'X':
				case 'x':
	...
	GUI_login() {
		...
		const res = this.model.authenticate(username, password);
    ...
    displayArticulos() {
		const articulos = this.model.getArticulos();
	...
	addArticulo() {
		...
		this.model.addArticulo(nombre, precio, stock);
	...
	editArticulo() {
		...
		if (this.model.editArticulo(id, nombre, precio, stock)) {
	...
	deleteArticulo() {
		...
		if (this.model.deleteArticulo(id)) {
	...
	buyArticulo() {
		...
		if (this.model.buyArticulo(id, cantidad)) {
			alert("Compra realizada.");
		} else {
			alert("No se pudo realizar la compra (stock insuficiente o ID inválido).");
		}
	}
}

window.onload = () => {
	const app = new ApplicationUI(new ApplicationModel());
	app.mainMenu();
};